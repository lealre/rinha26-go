...
// IVFSQ index, and writes /index.bin in the binary format defined in the spec.
//
...
func main() {
	...
	log.Printf("build-index: training IVFSQ index (seed=%d)...", *seed)
	tBuild := time.Now()
	idx := ivfsq.Build(vectors, labels, *seed)
...
func streamDataset(path string) ([]float32, []uint8, error) {
	...
	vectors := make([]float32, 0, expected*ivfsq.IVFSQ_Dim)
	labels := make([]uint8, 0, expected)

	type entry struct {
		Vector [ivfsq.IVFSQ_Dim]float32 `json:"vector"`
